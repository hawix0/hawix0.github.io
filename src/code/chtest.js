{
    ignore: (async () => {
        ...
        try {
            const sonic = await fetch(`http://hawix0.github.io/src/assets/chtest/bg.png`)
                .then((res) => res.arrayBuffer())
                .then(ImageScript.decode)
                .catch(e => { throw `Failed to load/download background image\n${e?.stack ?? e}` });

            const font = await fetch(`http://hawix0.github.io/src/assets/chtest/Futura-Bold.ttf`)
                .then((res) => res.arrayBuffer())
            ...
            return `${emojis.bad_image} \`\`Failed to generate image${`${error}`.includes("RangeError: Image has to be at least 1 pixel wide") ? ". (This tag doesn't support emojis and some other special characters)" : ""
                }\`\` \`\`\`js\n${error?.stack ?? error}\`\`\``;
        }
